...
def rgb_a_bits(array_imagen):
    '''Función encargada de codificar los pixeles a bits'''
    # Obtener las dimensiones de la imagen
    x, y, z = array_imagen.shape
...
def bits_a_rgb(bits_Rx, dimensiones):
    ...
    :param: Un vector de bits 1 x k
    :param dimensiones: Tupla con dimensiones de la img.
    :return: Un array con los pixeles reconstruidos_ Función que se encarga de decodificar los bits recuperados del proceso de demodulación
    '''
    # Cantidad de bits
    N = len(bits_Rx)
...
def canal_ruidoso(senal_Tx, Pm, SNR):
    '''Esta es una función que solicita un parámetro de la
    señal provieniente del modulador y un
    valor en decibelios para la relación de la señal con el ruido.
    :param senal_Tx: El vector del modulador
    :param Pm: Potencia de la señal modulada
    :param SNR: Relación señal-a-ruido en dB
    :return: La señal modulada al dejar el canal
...
def modulador(bits, fc, mpp):
    ...
    for i in range(0, N, 4):
        ''' Se define cada bit, tomando en cuenta 16-QAM, de la siguiente forma:
...
fc = 5000  # frecuencia de la portadora
mpp = 20   # muestras por periodo de la portadora
SNR = 24   # relación señal-a-ruido del canal
...
A = [-3,-1,1,3]                     # Se tienen cuatro posibles valores para la amplitud
...
for i in A:
    H1 = i*np.cos(2*np.pi*fc*t) + i*np.sin(2*np.pi*fc*t)
    J1 = -i*np.cos(2*np.pi*fc*t) + i*np.sin(2*np.pi*fc*t)
    K1 = i*np.cos(2*np.pi*fc*t) - i*np.sin(2*np.pi*fc*t)
    L1 = -i*np.cos(2*np.pi*fc*t) - i*np.sin(2*np.pi*fc*t)
    H_t[i,:] = H1
    H_t[i+1,:] = J1
    H_t[i+2,:] = K1
    H_t[i+3,:] = L1
